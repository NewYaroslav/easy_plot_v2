#ifndef EASY_PLOT_2_FILES_HPP_INCLUDED
#define EASY_PLOT_2_FILES_HPP_INCLUDED
...
namespace ep2 {
	 ...
	 * \return File size, will return -1 if the file could not be opened
	 */
	bool load_file(const std::string &file_name, std::vector<std::uint8_t> &file_data) {
		std::ifstream file(file_name, std::ios_base::binary);
		if(!file) return false;
		file.seekg(0, std::ios_base::end);
		std::ifstream::pos_type file_size = file.tellg();
		...
		file_data.reserve(file_size);
		std::uint8_t temp;
		while(file.read(reinterpret_cast<char *>(&temp), sizeof(std::uint8_t))) {
			file_data.push_back(temp);
		}
		file.close();
		return true;
	 ...
	 * \param file_name File name
	 * \param file_data File data
	 * \return Return false if file cannot be written
	 */
	bool write_file(const std::string &file_name, std::string &file_data) {
	 ...
	 * \return Return false if file cannot be written
	 */
	bool write_file(const std::string &file_name, std::vector<std::uint8_t> &file_data) {
		std::ofstream file(file_name, std::ios_base::binary);
		if (!file) return false;
		file.write(reinterpret_cast<char *>(file_data.data()), file_data.size());
		file.close();
		return true;
	}
	...
	std::string get_clear_path(const std::string &path) {
		std::vector<std::string> items;
		parse_path(path, items);
		std::string temp;
...
#endif // EASY_PLOT_2_FILES_HPP_INCLUDED
